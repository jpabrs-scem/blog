...
Azure VM Backup をオンラインでご利用いただくには対象の VM を起動しエージェントの状態を確認いただき、Ready となっていることが大前提 (必要条件) となりますので簡易的にはお客様にて下記 "エージェントの状態" をご確認ください。
サポート外の場合、Ready ではなく Not Ready となることが多いです。
![エージェントの状態 の確認](https://user-images.githubusercontent.com/71251920/154855287-696dca76-4bdc-4e8b-ac40-1108515edca8.png)
また、サポート対象の OS で Not Ready となっていた場合にはエージェントが正しく動いていないため、Azure VM Backup が失敗する場合には、agent の再起動や 対象マシンの再起動を行っていただき、それでも事象が改善しない場合には Azure VM agent イシューとしてお問い合わせいただければと存じます。
