...
今回はお問い合わせをいただくことが多い、 "クロスリージョン リストア を実施した際にセカンダリ リージョン (ペアリージョン) へリストアできない、` ` `Error Code : UserErrorVmProvisioningFailedDueToLackOfResources` ` `  で失敗する" といったお問い合わせについて解説させていただきます。
