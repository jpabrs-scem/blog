...
1. 以下の画面が出た場合には、``Agree`` を選択します。  
    ![PsExec License Agreement](./How_To_SQL_Backup_Via_Proxy/How_To_SQL_Backup_Via_Proxy_01.png)

1. Internet Explorer が起動されるため、``[Tools] > [Internet Options] > [Connections] > [LAN settings]``の順に移動します。  
    ![](./How_To_SQL_Backup_Via_Proxy/How_To_SQL_Backup_Via_Proxy_02.png)

1. ``Proxy Server`` 欄の以下項目のチェックを有効化し、Proxy Server の IP およびポート番号を入力します。  
   ``Use a proxy server for your LAN …``
    ![](./How_To_SQL_Backup_Via_Proxy/How_To_SQL_Backup_Via_Proxy_03.png)
...
1. ``Proxy Server`` 欄の以下項目のチェックを有効化します。  
    ...
    ![](./How_To_SQL_Backup_Via_Proxy/How_To_SQL_Backup_Via_Proxy_04.png)

1. ``Proxy Server`` 欄の ``[Advanced]`` を選択します。
    ![](./How_To_SQL_Backup_Via_Proxy/How_To_SQL_Backup_Via_Proxy_05.png)
...
1. ``Exceptions`` 欄に、プロキシから除外する IP アドレスおよび FQDN を入力します。  
    ...
    ![](./How_To_SQL_Backup_Via_Proxy/How_To_SQL_Backup_Via_Proxy_06.png)
...
1. レジストリ エディターを開き、以下パスに遷移し、サービス アカウント ( NT Service\AzureWLBackupPluginSvc ) が作成されていることを確認します。  
   ...
   ![](./How_To_SQL_Backup_Via_Proxy/How_To_SQL_Backup_Via_Proxy_07.png)
...
1. 現在ログインしているユーザー (カレント ユーザー) で、``[設定] > [ネットワークとインターネット] > [プロキシ]`` を開き、一時的に Proxy Server を設定します。  
   上記項目 2 と同じプロキシ設定を行ってください。  
    ![](./How_To_SQL_Backup_Via_Proxy/How_To_SQL_Backup_Via_Proxy_08.png)
...
1. 管理者特権の PowerShell から、ダウンロードしたファイルを解凍したフォルダに移動し、次のコマンドを実行します。
    ...
    ![](./How_To_SQL_Backup_Via_Proxy/How_To_SQL_Backup_Via_Proxy_09.png)
...
1. 「Start-ConnectivityTests.ps1」を実行した後、ターミナル上に出力された結果に問題が無いことを確認します。
    ...
    ![](./How_To_SQL_Backup_Via_Proxy/How_To_SQL_Backup_Via_Proxy_10.png)
